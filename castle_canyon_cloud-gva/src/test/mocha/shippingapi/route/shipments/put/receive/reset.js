...
function getRoutePath(id) {
  return `${routePath}/${id}/receive/reset`;
}

describe('transition shipment from inReceiving back to inMonitoring', function() {
  ...
  it('should require status lock owner', async function() {
    ...
    res = await helper.shipment.receive.reset(app, token, id);
  ...
  it('should update shipment', async function() {
     ...
     */
    res = await helper.shipment.getById(app, token, id);
    helper.shipment.expectStatusAndUnlocked(res, statusConst.inMonitoring());
